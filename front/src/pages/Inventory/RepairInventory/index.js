...
import { getQuarantine, sendToScrap, sendToInventory, getRepair } from '../../../Services/InventoryService'
...
const RepairInventory = () => {
    ...
    const loadItems = (page,filter, value) => {
        getRepair(page,filter, value).then(resp => {
    ...
    // })
        ...
        const successDeleteDialog = (value) => {
            if(itemToDelete.id !== null && itemToDelete.id !== undefined){
                let promise = value == 1 ? sendToInventory(itemToDelete.id) : sendToScrap(itemToDelete.id);
                promise.then(resp => {
                    loadItems(1)
                }).catch(err => {
                    console.error(err.response)
            ...
            <MDBox my={3}>
                ...
                <Card>
                    { items.rows !== undefined && items.rows.length > 0 && 
                        <><DataTable useActions useDelete useTime table={items} showTotalEntries={false} entriesPerPage={false} 
                              handleSearch={handleSearch} canSearch timeAction={timeAction} deleteAction={deleteRegister}/> 
