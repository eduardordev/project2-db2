...
import {inventoryMissingExpiration} from "../../../utils/tableHeaders"
import { getMissingExpiration, sendToScrap } from '../../../Services/InventoryService'
...
const MissingExpiration = () => {
    ...
    const loadItems = (page, filter, valueFilter) => {
        getMissingExpiration(page, filter, valueFilter).then(resp => {
            console.log(resp.data)
            setItems(buildData(resp.data.inventory_detail,inventoryMissingExpiration()))
    ...
    const editPiece = (item => {
        window.location.replace("/inventory/missing_expitarion/update/".concat(item.id))
    })
    const viewPiece = (item => {
        window.location.replace("/inventory/missing_expitarion/view/".concat(item.id))
    })
    ...
    const successDeleteDialog = () => {
        sendToScrap(itemToDelete.id).then((resp) => {
            loadItems(1)
        }).catch((err) => {
            console.error(err)
        });
    ...
    return(
        <div>
            ...
            <MDBox my={3}>
                ...
                <Card>
                    { items.rows !== undefined && items.rows.length > 0 && 
                        <><DataTable useActions useEdit useView useDelete useTime deleteAction={deleteRegister}  deleteMessage="Desechar"  table={items} showTotalEntries={false} entriesPerPage={false} 
                        editAction={editPiece} viewAction={viewPiece} handleSearch={handleSearch} canSearch timeAction={timeAction}/> 
            ...
            <DeleteDialog  open={openDeteleDialog} message="Esta seguro de querer desechar este item?" successCalback={successDeleteDialog} cancelCallback={closeDeleteDialog} />
...
export default MissingExpiration;